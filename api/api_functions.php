...
function send_membership_confirmation_mail($user, $first_user_membership) {
    $expires_msg = isset($user['expires']) ? "Det varer helt til ".$user['expires'].". " : "";
    if( !isset($user['email']) ) {
        // No email set, bail.
        return;
    }
    ...
    $message .= '<p style="margin-bottom: 20px;">'.$expires_msg.'Trykk p&aring; lenken under for &aring; logge inn og se medlemskapet ditt.</p>';
