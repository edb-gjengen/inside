...
class MemberCard {
  ...
  function __construct() {
    ...
    $this->selectStmt = self::$PDO->prepare(
      "SELECT * FROM din_membercard WHERE id=?");
    $this->findByUserIdStmt = self::$PDO->prepare(
      "SELECT * FROM din_membercard WHERE userId=?");
    $this->insertStmt = self::$PDO->prepare(
      "INSERT INTO din_membercard (userId, ordered, produced, delivered, active) VALUES (?, ?, ?, ?, ?)");
    $this->updateStmt = self::$PDO->prepare(
      "UPDATE din_membercard SET 
      	userId=?,
      	ordered=?,
      	produced=?,
      	delivered=?,
    ...
    $this->deleteStmt = self::$PDO->prepare(
      "DELETE FROM din_membercard WHERE id=?");
